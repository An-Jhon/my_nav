          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {getCurrentLinks().map(link => (
              <a
                ...
                className="group flex items-start p-3 bg-white rounded-lg border hover:shadow-md transition-all duration-200 relative hover:border-blue-200"
              >
                <FavoriteButton
                  id={link.id}
                ...
                />
                <div className="mr-3 w-6 h-6 flex-shrink-0 flex items-center justify-center">
                  <img
                    src={getFaviconUrl(link.url)}
                    alt={`${link.title} icon`}
                    width={24}
                    height={24}
                    className="rounded-sm group-hover:scale-110 transition-transform duration-200"
                    onError={(e) => {
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500 line-clamp-2 mt-0.5 group-hover:text-gray-600">
                    {link.description}
                  </p>
                </div>
              </a>
