...
export default function HomePage() {
  ...
  const handleReorderFavorites = (reorderedLinks: NavLink[]) => {
    const newOrder = reorderedLinks.map(link => link.id)
  ...
  const getCurrentLinks = () => {
    if (currentCategory === 'favorites') {
      ...
      return favoriteOrder.length > 0
        ? favoriteOrder
            .map(id => favoriteLinks.find(link => link.id === id))
            .filter((link): link is NavLink => link !== undefined)
            .concat(favoriteLinks.filter(link => !favoriteOrder.includes(link.id)))
        : favoriteLinks
    }
