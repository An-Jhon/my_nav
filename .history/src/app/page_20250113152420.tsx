...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (savedOrder) {
      ...
      console.log('Loaded favorite order:', savedOrder)
    }
  ...
  const getCurrentLinks = (): NavLink[] => {
    if (currentCategory === 'favorites') {
      ...
      console.log('Favorite links before sorting:', favoriteLinks)
      const sortedLinks = favoriteLinks.sort((a, b) => {
        const indexA = favoriteOrder.indexOf(a.id)
        const indexB = favoriteOrder.indexOf(b.id)
        console.log(`Comparing ${a.id}(${indexA}) with ${b.id}(${indexB})`)
        if (indexA === -1) return 1
        if (indexB === -1) return -1
        return indexA - indexB
      })
      console.log('Sorted favorite links:', sortedLinks)
      return sortedLinks
    }
